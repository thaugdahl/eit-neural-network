...
def get_predictions(starting_window, time_step, nn, N, injection_layers=None):
    """
    ...
    for i in range(N):
        if injection_layers:
            prediction = nn.predict(x=[np.array([last_window]), create_injection_data(injection_layers,np.array([last_window]))[0]])[0]
        else:
